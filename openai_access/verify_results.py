...
from friday.api_connector import ApiConnector
...
def read_keys():
    file = open("/home/wangshuhe/gpt-ner/openai_access/all_keys/100.5.keys.txt", "r")
    keys = []
    for line in file:
...
def multi_ner_access(api_connector, inference_parameters, ner_pairs):
    print("tagging ...")
    results = api_connector.get_multiple_sample_in_parallel(
        prompt_list=ner_pairs,
...
if __name__ == '__main__':
    ...
    verify_results = multi_ner_access(api_connector=client, inference_parameters=inference_parameters, ner_pairs=prompts)
